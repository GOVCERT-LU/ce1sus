...
from ce1sus.db.classes.cstix.common.related import RelatedThreatActor, RelatedIndicator, RelatedPackageRef, RelatedTTP, RelatedObservable
...
_REL_INCIDENT_TIME = Table('rel_incident_time', getattr(Base, 'metadata'),
...
class Incident(BaseCoreComponent, Base):
  ...
  time = relationship(Time, secondary=_REL_INCIDENT_TIME, uselist=False)
  ...
  related_incidents = relationship(RelatedIndicator, secondary=_REL_INCIDENT_RELATED_INCIDENT)
  ...
  # ce1sus specific
  event = relationship('Event', uselist=False, primaryjoin='Incident.event_id==Event.identifier')
