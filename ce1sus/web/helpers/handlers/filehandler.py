...
class FileHandler(GenericHandler):
  ...
  def populateAttributes(self, params, obj, definition, user):
    ...
    if isfile(filepath):
      ...
      attributes = list()
      attributes.append(self.__createAttribute(basename(filepath), obj, 'filename', user))
      attributes.append(self.__createAttribute(hasher.fileHashMD5(filepath), obj, 'md5', user))
      sha1 = self.__createAttribute(hasher.fileHashSHA1(filepath), obj, 'sha1', user)
      attributes.append(sha1)
      attributes.append(self.__createAttribute(hasher.fileHashSHA256(filepath), obj, 'sha256', user))
      attributes.append(self.__createAttribute(hasher.fileHashSHA384(filepath), obj, 'sha384', user))
      attributes.append(self.__createAttribute(hasher.fileHashSHA384(filepath), obj, 'sha384', user))
      attributes.append(self.__createAttribute(hasher.fileHashSHA512(filepath), obj, 'sha512', user))
      attributes.append(self.__createAttribute('{0} Bytes'.format(getsize(filepath)), obj, 'size', user))
      url = pathname2url(filepath)
      mime = MimeTypes()
      attributes.append(self.__createAttribute(unicode(mime.guess_type(url)[0]), obj, 'mimeType', user))
      ...
      attributes.append(self.__createAttribute(destination, obj, 'location', user))
  ...
  def __createAttribute(self, value, obj, definitionName, user):
    ...
    attribute.definition = self.def_attributesBroker.getDefintionByName(definitionName)
