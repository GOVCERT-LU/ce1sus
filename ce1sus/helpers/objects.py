...
def compareObjects(object1, object2):
  ...
  if (type(object1) != type(object2)) and not (isinstance(object1,
    ...
    elif ((isinstance(object1, types.StringTypes) or isinstance(object1, types.IntType)
                or isinstance(object1, types.FloatType) or isinstance(object1, types.LongType)
                or isinstance(object1, types.TupleType)) and
                (isinstance(object2, types.StringTypes) or isinstance(object2, types.IntType)
                or isinstance(object2, types.FloatType) or isinstance(object2, types.LongType)
  ...
  if not result:
    if (typeIssue):
      ...
      print 'Attribute {0} is not equal, got {1} expected {2}'.format(attrName, attrValue1, attrValue2)
  ...
  for name, fqname in vars(type(obj)).iteritems():
    ...
    if not name.startswith('_'):
      ...
      if (not isfunction(value) and not isinstance(value, types.FunctionType)
        ...
        if isinstance(value, Iterable) and not isinstance(value, types.StringTypes):

          if len(value) == 0:
            print '{indentation}{variableName}: Empty'.format(indentation=indentStr,
                                                                            variableName=name)
          else:
            for item in value:
              print '{indentation}{variableName} {type} :'.format(indentation=indentStr, variableName=name, type=type(item))
              printObject(item, indent + 1, maxRecLVL)
        elif isinstance(value, object):
          # It should be one base type only the following ones are supported to give a direct output
          if (isinstance(value, types.StringTypes) or isinstance(value, types.IntType)
              or isinstance(value, types.FloatType) or isinstance(value, types.LongType)
              or isinstance(value, types.TupleType) or type(value) == datetime.datetime):
            print '{indentation}{variableName}\t: "{variableValue}"'.format(indentation=indentStr,
                                                                            variableName=name, variableValue=value)
          else:
            # Ok it is a custom object
            print '{indentation}{variableName} {type} :'.format(indentation=indentStr, variableName=name, type=type(value))
